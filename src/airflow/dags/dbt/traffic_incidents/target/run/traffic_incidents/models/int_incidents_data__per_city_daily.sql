    ...
    create or replace table `central-catcher-448814-j1`.`traffic_incidents_253`.`int_incidents_data__per_city_daily`
    ...
    OPTIONS()
    as (
      -- Aggregate incidents per city (daily basis)
...
from incidents
group by occurence_date, incident_cause, city
),
pivoted_by_cause as (
select 
		*
...
join pivoted_by_cause p
	on t.occurence_date = p.occurence_date and t.city = p.city
    );
